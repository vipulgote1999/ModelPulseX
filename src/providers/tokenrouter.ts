...
function isFreeTokenRouterId(id: string): boolean {
  ...
  return (
    lower.endsWith(":free") || lower.endsWith("-free") || lower.endsWith("free")
  );
}
...
const FALLBACK: Array<{ id: string; name: string; ctx: number }> = [
  // Verified free from live TokenRouter catalog (ending with :free / -free)
  {
    ...
    name: "Nemotron 3 Nano Omni 30B Reasoning (free)",
    ctx: 256000,
  },
...
export class TokenRouterProvider implements LLMProvider {
  ...
  async discoverModels(): Promise<ModelMetadata[]> {
    try {
      ...
      const r = await fetch(MODELS_URL, {
        headers: this.env.TOKENROUTER_API_KEY
          ? { authorization: `Bearer ${this.env.TOKENROUTER_API_KEY}` }
  ...
  async benchmarkModel(
    ...
    return measureBenchmark({
      ...
      apiUrl: CHAT_URL,
      apiKey: this.env.TOKENROUTER_API_KEY as string | undefined,
      benchmark: b,
    });
  }
}
