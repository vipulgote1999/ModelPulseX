...
export default function Docs() {
  ...
  if (error)
    return (
      <div className="max-w-[900px] mx-auto px-4 sm:px-6 py-8 text-amber-300">
        Failed to load API docs: {error}
      </div>
    );
  if (!spec)
    return (
      <div className="max-w-[900px] mx-auto px-4 sm:px-6 py-8 text-zinc-400 animate-pulse">
        Loading API docs…
      </div>
    );
    ...
    <div className="max-w-[900px] mx-auto px-4 sm:px-6 py-8 space-y-8 text-zinc-300 leading-relaxed">
      <div>
        ...
        <p className="text-xs text-zinc-500 mt-1">
          OpenAPI {spec.openapi} · version {spec.info.version} · server{" "}
          <span className="mono text-zinc-300">{base}</span>
        </p>
      </div>
      ...
      <section className="rounded-xl border border-zinc-800 bg-zinc-900/40 p-5 space-y-3">
        ...
        <p className="text-sm text-zinc-500">
          All public GET endpoints are unauthenticated. Admin endpoints require{" "}
          <code className="mono text-xs bg-zinc-800 px-1 py-0.5 rounded">
                  ...
                  <table className="w-full text-xs border border-zinc-800 rounded-lg overflow-hidden">
                    <thead className="bg-zinc-900 text-zinc-400 text-[11px] tracking-widest">
                      <tr>
                        ...
                        <th className="text-left px-2 py-1">In</th>
                    ...
                    <tbody>
                      {(
                        ...
                        <tr key={p.name} className="border-t border-zinc-800">
                          <td className="px-2 py-1 mono text-zinc-300">
                            {p.name}
                          </td>
                          ...
                          <td className="px-2 py-1 mono text-zinc-500 text-[11px]">
      ...
      <section className="rounded-xl border border-zinc-800 bg-zinc-900/40 p-5 space-y-2">
        ...
        <p className="text-sm">
          Embed the live leaderboard as an image:{" "}
          <code className="mono text-xs bg-zinc-950 border border-zinc-800 rounded px-1.5 py-0.5">
            {base}/api/og.png
          </code>
          . Returns 1200×630 PNG, dark theme, top 5 models, 5-minute cache.
        </p>
        <img
          src="/api/og.png"
          alt="OG preview"
          className="rounded-lg border border-zinc-800 w-full max-w-[600px] aspect-[1200/630] object-cover bg-[#0a0a0f]"
          onError={(e) =>
