...
export default function Leaderboard({
  ...
  const [sortKey, setSortKey] = useState<keyof Row | "overall_score">(
    "overall_score",
  );
  ...
  const sort = (k: keyof Row | "overall_score") => {
    if (sortKey === k) setDir((d) => (d === "desc" ? "asc" : "desc"));
      ...
      setSortKey(k);
      setDir(
        k === "ttft_now" || k === "ttft_7d" || k === "itl_now" || k === "itl_7d"
          ? "asc"
          : "desc",
      );
    }
  ...
  useEffect(() => {
    fetch("/api/providers")
      .then((r) => r.json())
      .then((j: unknown) => {
        const list = ((j as { providers?: Array<Record<string, unknown>> })
        ...
        for (const p of list) {
          ...
          const ft = p["freeTier"] as
            ...
            | undefined;
          const usage24h =
            typeof p["usage24h"] === "number"
              ? (p["usage24h"] as number)
              : undefined;
          if (ft || usage24h !== undefined)
            m.set(name, { ...ft, usage24h } as never);
          ...
          <thead className="bg-zinc-900/80 text-[11px] tracking-widest text-zinc-400 border-b border-zinc-800">
            <tr>
              ...
              <th
                ...
                onClick={() => sort("tps_now")}
              >
                TPS Now
              </th>
              <th
                className="text-right px-2 py-2 hidden xl:table-cell cursor-pointer hover:text-white"
                onClick={() => sort("tps_1h")}
              >
                1h
              </th>
              ...
              <th
                className="text-right px-2 py-2 hidden lg:table-cell cursor-pointer hover:text-white"
                onClick={() => sort("uptime_7d")}
              >
                7d Up
              </th>
              {showAA && (
              ...
              <th className="text-left px-2 py-2 hidden 2xl:table-cell">
                Trend (24h)
              </th>
              ...
              <th className="text-left px-3 py-2 hidden 2xl:table-cell">
                Last Test
              </th>
            </tr>
          </thead>
          <tbody>
            {sorted.length === 0 && (
              <tr>
                ...
                >
                  No free models discovered yet — discovery runs hourly + on
                  deploy. Check{" "}
                  <span className="text-zinc-300">/api/models</span>.
                </td>
              </tr>
            )}
            {sorted.map((r) => (
              ...
              >
                ...
                <td className="px-3 py-2">
                  ...
                  {!r.active && (
                    <span className="inline-flex text-[10px] px-1.5 py-0.5 rounded bg-amber-900/40 text-amber-300 border border-amber-800">
                      Previously Free
                    </span>
                  )}
                  {r.free_status !== "FREE" && r.active === false && (
                ...
                <td className="px-3 py-2 hidden md:table-cell">
                  ...
                  {r.overall_score != null && (
                    <span className="ml-2 text-[11px] text-zinc-500 hidden xl:inline">
                      {r.overall_score}
                    </span>
                  )}
                  {(() => {
                    ...
                    if (parts.length === 0 && lim.tokensPerDay)
                    ...
                    return (
                      <div
                        title={title || label}
                        className="mt-1 text-[10px] px-1.5 py-0.5 rounded bg-zinc-800/60 text-zinc-400 border border-zinc-700 inline-block"
                      >
                        {label}
                      </div>
                    );
                ...
                <td className="px-2 py-2 text-right mono font-medium">
                  {fmtTps(r.tps_now)}
                </td>
                <td className="px-2 py-2 text-right mono hidden xl:table-cell text-zinc-400">
                  {fmtTps(r.tps_1h)}
                </td>
                <td className="px-2 py-2 text-right mono hidden xl:table-cell text-zinc-400">
                  {fmtTps(r.tps_24h)}
                </td>
                <td className="px-2 py-2 text-right mono text-zinc-200">
                  {fmtTps(r.tps_7d)}
                </td>
                ...
                <td className="px-2 py-2 text-right mono hidden lg:table-cell">
                  {r.uptime_7d != null
                    ? `${(r.uptime_7d * 100).toFixed(1)}%${r.sampleCount24h != null && r.sampleCount24h < 12 ? ` n=${r.sampleCount24h}` : ""}`
                    : "—"}
                </td>
                {showAA && (
                    ...
                    {(() => {
                      ...
                      return aa ? (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="text-xs px-1.5 py-0.5 rounded bg-zinc-800 border border-zinc-700 text-zinc-300 hover:text-white hover:border-zinc-500"
                ...
                <td className="px-2 py-2 text-right mono hidden sm:table-cell text-zinc-400">
                  {r.error_rate_7d != null
                    ? `${(r.error_rate_7d * 100).toFixed(1)}%`
                    : "—"}
                </td>
                  ...
                  <div className="flex flex-col gap-0.5">
                    <div className="flex gap-1 items-center flex-wrap max-w-[120px] xl:max-w-[160px]">
                      ...
                      {(() => {
                        ...
                        const pc = providerCdMap.get(r.provider);
                        if (pc)
                          return (
                            <span
                              title={`${pc.reason ?? "PROVIDER_COOLDOWN"} until ${new Date(pc.cooldown_until).toLocaleString()}`}
                              className="text-[10px] px-1.5 py-0.5 rounded bg-amber-900/30 text-amber-300 border border-amber-800"
                            >
                              ⏱ provider {remainingStr(pc.cooldown_until)}
                            </span>
                          );
                        return null;
                      })()}
